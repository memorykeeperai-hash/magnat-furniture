...
export const metadata = {
  title: 'Dining Room Collection - MAGNAT Furniture',
  description: 'Explore our premium dining room furniture collection.',
};

export default async function DiningRoomPage() {
  const products = await getProducts();
  let roomProducts = products.filter(
    p => typeof p.room === 'string' && p.room.toLowerCase().includes('dining') && Array.isArray(p.images) && p.images.length > 0
  );
        ...
        {roomProducts.length > 0 ? (
          ...
          <FadeInView className="flex flex-col items-center justify-center py-12 md:py-16 px-4 text-center">
            ...
            <p className="text-[#666] mb-10 max-w-md mx-auto text-base font-light leading-relaxed">
              We are currently curating new pieces for our Dining Room collection. Please check back soon or explore our available pieces.
            </p>
