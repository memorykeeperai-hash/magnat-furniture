...
export const metadata = {
  title: 'All Room Pieces - MAGNAT Furniture',
  description: 'Explore our complete collection of room furniture.',
};

export default async function AllRoomPiecesPage() {
  const products = await getProducts();
  let roomProducts = products.filter(
    p => typeof p.room === 'string' && p.room.trim() !== '' && Array.isArray(p.images) && p.images.length > 0
  );
        ...
        {roomProducts.length > 0 ? (
          ...
          <FadeInView className="flex flex-col items-center justify-center py-12 md:py-16 px-4 text-center">
            ...
            <p className="text-[#666] mb-10 max-w-md mx-auto text-base font-light leading-relaxed">
              We are currently curating our overall furniture collection. Please check back soon or contact us directly for bespoke designs.
            </p>
            <a href="/contact" className="inline-flex items-center justify-center bg-[#111] text-white text-[10px] font-bold tracking-[0.2em] uppercase px-8 py-4 rounded-[4px] hover:bg-[#C0001A] transition-colors duration-300">
              Contact Us
            </a>
          </FadeInView>
        )}
